#!/usr/bin/env bash
# agent — uninstaller. Removes symlinks recorded by install.sh, per target.
set -euo pipefail
...
usage() {
  cat <<EOF
${B}agent — uninstaller${R}
Removes the symlinks install.sh created, based on the manifest at
.agent-links.json (gitignored). Safety rules: a link is removed only if it
