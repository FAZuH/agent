...
status: confirmed
source: live-test
evidence: `plugins/mermaid/src/doctor-core.ts` loaded playwright via `createRequire(path.join(dir, "__doctor__.js"))("playwright")`. The service reported "playwright not resolvable" for every candidate dir — including `/home/fazuh/.config/opencode/node_modules` AFTER playwright 1.63.0 was copied there as real directories (symlinks ruled out first), where `node -e "require.resolve(...)"` resolves it and a plain `bun` process resolves AND loads it (`chromium` export present). Another installed plugin's dependency (`effect`) does load from that same dir, but that is a static import handled by the plugin loader. Switching to `await import(pathToFileURL("<dir>/playwright/index.mjs").href)` resolved and launched from inside the same in-process code path.
