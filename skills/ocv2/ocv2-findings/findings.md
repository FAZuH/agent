...
CORRECTION 2026-09-15: the promise-wrapper shapes above are right, but "trust the binary over `@opencode-ai/plugin` types" was wrong for PROPS — `plugin/dist/tui.d.ts` types every dialog's props accurately (the earlier check looked only in `index.d.ts`, which does not carry them). Read that file first; grepping the compiled bundle instead cost a shipped crash. See the `dialog.select` entry below.
...
status: confirmed
source: live-test
evidence: 2026-08-25 built a working graded-question form on it; 2026-08-27 re-probed after beta broke `label`: POST /api/session/:sid/form with `fields:[{key,type,label}]` silently drops `label` (persisted as {key,type,options} only, verified via `ses_fbd7c5dd6ffedKdGWOxUh2MDus` frm_04283bcae001… vs frm_04283beb4001…), while `fields:[{key,type,title,description}]` persists correctly; OpenAPI `GET /openapi.json` `Form.StringField`/`Form.MultiselectField`/`Form.CreatePayload` now require `title`+ optional `description` (no `label` at field level; `Form.Option.label` still valid). Create: POST /api/session/:sid/form {title, fields:[{key, type: "string"|"multiselect"|"boolean"|"integer"|"number", title, description?, options:[{label,value,description?}]}]} → {data:{id:"frm_…"}}; string+options renders as single-select, multiselect as multi. State: GET …/form/:fid/state → {status:"pending"|"answered"|"cancelled", answer:{key:value}}. Reply: POST …/form/:fid/reply {answer:{key:value}} → 204; partial answers (omitting optional fields) accepted; settled forms reject with FormAlreadySettledError. Cancel: POST …/form/:fid/cancel. List: GET /api/session/:sid/form (settled forms drop off). The BUILT-IN TUI renders arbitrary pending session forms generically — a plugin-created form shows up and is answerable with no custom TUI code. Built-in question tool input shape: {questions:[{question, header, options:[{label,description}]}]}.
GOTCHAS: (1) server-plugin ctx has NO form/client API (ctx keys: app,options,agent,aisdk,catalog,command,event,integration,mcp,plugin,reference,skill,storage,tool,websearch,session,shell) — call the forms API via `opencode2 api post <path> -d <json>` subprocess. (2) The headless `opencode2 run` client auto-DISMISSES pending forms ("The user dismissed this question") — answered-path testing must drive sessions via raw HTTP (POST /api/session, POST /api/session/:sid/prompt). (3) The TUI displays raw tool-call arguments while a tool runs: NEVER put a correct answer in a tool's input — split ask/grade into two tools at the answer boundary. (4) Backgrounded `opencode2 run` processes die with their parent shell ("Session interrupted: shutdown") — keep the shell waiting on the PID, or drive the whole flow over HTTP. (5) Field `label` → `title`/`description` rename broke a plugin-built form (only form `title: firstLine(question)` rendered; field question vanished, TUI fell back to `key` name like "answer") — fixed 2026-08-27 by mapping `question→title` + `details→description`. (6) Tool `execute.before` hooks see pre-execution input, so any input-derived display transform (e.g. option shuffle) is invisible to mirroring hooks — keep such transforms deterministic (alphabetical display order) so every surface computes the same order.

## [2026-09-13] plugins: no setup ctx path is usable as a string — use ctx.location.directory
status: confirmed
source: live-test
evidence: 2026-08-25 — learn-viz-tools published PNGs to /home/<user>/viz instead of the session project after its `-g` install; setup() captures its ctx once. 2026-09-13 (opencode v2.0.3) — key-dumped the setup ctx of a PROJECT-LOCAL plugin (`/home/<user>/Work/Notes/.opencode/plugins/notes-plugin.js`): `ctx.worktree` = `{}` (an empty OBJECT, therefore truthy), `ctx.directory` = `undefined`, `process.cwd()` = `/home/<user>`; the transform-tool `execute()` second arg keys are exactly `[sessionID, agent, messageID, id, progress]` — no worktree, no directory; `ctx.location` = `{directory: "/home/<user>/Work/Notes", project: {id, directory, canonical}}`. The vault plugin's `const directory = \`${ctx.worktree || ctx.directory || process.cwd()}/\`` stringified that empty object and wrote every note to `$HOME/[object Object]/3 Reference/…` while reporting success. The GLOBAL load is the same shape, proven by live failure rather than inference: `mermaid-compile` (installed at `~/.config/opencode/plugins/mermaid`) threw `The "paths[0]" property must be of type string, got object` on EVERY call (its `baseDir = ctx.worktree || ctx.directory` was `{}`, and `outDir` is always resolved against it), and `mermaid-doctor` printed `[object Object]` as its first playwright candidate dir.
v2 plugin setup ctx carries no usable project path as a string. Read **`ctx.location.directory`** — and note that for a GLOBALLY installed plugin that is the service's default location, not the user's project, so a plugin that writes into "the project" must resolve it per call from `GET /api/session/<sid>` → `data.location.directory` (async; see the execFileSync self-deadlock entry). The original advice here — "keep tctx.worktree/directory as first preference for project-local loading" — is RETRACTED: those keys do not exist on the transform-tool execute ctx, and `ctx.worktree` is a truthy `{}`, so the `worktree || directory || cwd` idiom always resolves to the object and interpolates to `[object Object]` (or blows up in `path.resolve`). Fixed 2026-09-13 in `plugins/mermaid/src/index.ts` (both tools, plus a per-session cache) and `plugins/viz/src/index.ts` (`fallbackBase`, and the dead `tctx?.worktree || tctx?.directory` branch removed).
VERIFY A PLUGIN WITHOUT RESTARTING THE SERVICE: nested `src/` edits do not hot-reload, but you can prove the code from a throwaway `bun` process — `import mod from ".../src/index.ts"`, call `await mod.setup(fakeCtx)` with the real v2 shape (`{worktree: {}, location: {directory: "/home/<user>"}, tool: {transform: fn => fn({add: t => registered.push(t)})}}`), then `await registered.find(t => t.name === "…").execute(input, {sessionID: "<a live session id>"})`. The session-API branch runs for real, so relative paths come back resolved against that session's project.
That recipe is server-plugin only. A TUI plugin cannot be set up against a fake
ctx — `ctx.ui` is the live Solid host (dialog, toast, theme) — so its no-restart
checks are: unit-test the pure exported helpers under `bun test`, then
`bun -e 'import("./tui.ts")'` as an import smoke test. The gap bites: a wrong
dialog prop is not a caught error, it takes the whole TUI down at render (see
the 2026-09-15 dialog.select entry).
The ToolContext .d.ts (worktree, directory, abort, metadata, ask) documents the legacy tool() helper's context, NOT the transform-tool execute arg — don't trust it for transform tools (re-confirmed: `@opencode-ai/plugin@1.18.21` still types both as strings).
...
UPDATE 2026-09-13 (v2.0.3): the requirement is conditional on importing the SDK. The Notes vault plugin now imports only `fs`, `path` and a relative `../core/index.js`, its `.opencode/package.json` has `"dependencies": {}`, and it loads + hot-reloads fine with the dep absent from the manifest — a zero-SDK-import project plugin needs no install and no `min-release-age` bypass. Don't re-add `@opencode-ai/plugin` to satisfy a stale note; the plain `{id, setup}` shape needs nothing from it.
...
status: superseded (wrong for v2.0.3 — see the 2026-09-13 entry)
source: live-test
evidence: on beta-19157-era builds, rewriting `plugins/mermaid/src/index.ts` and touching the entry re-logged `loading plugin` with a fresh entry `?mtime=` yet the call still failed with the pre-fix error; only killing the port-holder service picked up the nested change. That conclusion held for that build and is what made me distrust hot reload for a week — see the 2026-09-13 entry directly below for the corrected behaviour and the test that settles it either way.

## [2026-09-13] plugins: every file under a plugin dir hot-reloads, nested src included
status: confirmed
source: live-test
evidence: on v2.0.3 (service started 17:52, before the edit) a marker string inserted in the NESTED `plugins/mermaid/src/doctor-core.ts` report line — not the entry — reached the live service after `sync.sh push -g plugins` with no restart: `mermaid-doctor` returned `[engine-HOTMARKER: chromium via playwright@…]`. Reverting + pushing again restored the original text, also live. Each pushed file triggers its own reload (the log showed four `msg="loading plugin"` events 27ms apart after one push of a 4-file plugin, entry URL with no `?mtime=` cache-buster), and the `message="watcher subscribe"` lines do list the nested `src/*.ts` paths, which is what a recursive watch looks like.
So: after editing any plugin file, push and re-call the tool — no restart. When a change seems absent, suspect the session's cached tool catalog or a stale installed copy (`sync.sh diff`), not module caching, and prove which build is live with a behaviour marker (a distinctive string in output) rather than trusting a log line. The earlier "nested stays stale" rule was a real observation of an older build, not a v2 law.
...
## [2026-09-13] plugins: inside the compiled opencode2 binary `createRequire()` cannot see on-disk packages — `import()` with an absolute file URL can
status: confirmed
source: live-test
evidence: `plugins/mermaid/src/doctor-core.ts` loaded playwright via `createRequire(path.join(dir, "__doctor__.js"))("playwright")`. The service reported "playwright not resolvable" for every candidate dir — including `/home/<user>/.config/opencode/node_modules` AFTER playwright 1.63.0 was copied there as real directories (symlinks ruled out first), where `node -e "require.resolve(...)"` resolves it and a plain `bun` process resolves AND loads it (`chromium` export present). Another installed plugin's dependency (`effect`) does load from that same dir, but that is a static import handled by the plugin loader. Switching to `await import(pathToFileURL("<dir>/playwright/index.mjs").href)` resolved and launched from inside the same in-process code path.
A server plugin that needs an npm package outside opencode's own bundle cannot use `createRequire()`/bare specifiers at call time: the compiled binary's require layer does not walk real node_modules chains the way node/bun do. Resolve the package's own entry file and `import()` it by absolute file URL (`index.mjs`/`index.js` under `<node_modules>/<pkg>/`), awaiting it — the plugin host is async. And put every candidate's rejection reason in the thrown message: an empty `catch {}` here made a working install look missing for three rounds of probes.
Related trap in the same code: playwright's DEFAULT headless launch wants the separate `chrome-headless-shell` build, which is a second ~114 MB download on top of `chromium` — if it is missing, `{ headless: true, channel: "chromium" }` runs the full build instead and renders identically (verified: same page, byte-identical PNG). Both builds are installed here now, so doctor uses the default. The download also reports success when it is cut short: piping `playwright install` through `tail` makes the shell exit 0 while the target dir is absent, so check for `~/.cache/ms-playwright/chromium_headless_shell-<build>/` rather than trusting the exit code.

## [2026-09-13] plugins: a synchronous `opencode2 api` call inside a tool execute() self-deadlocks
status: confirmed
source: live-test
evidence: the Notes vault create-note fix called `execFileSync("opencode2", ["api", "get", `/api/session/${sid}`], { timeout: 10_000 })` from inside `ctx.tool.transform(...)`'s `execute()`; every call burned the full 10 s and failed with `spawnSync opencode2 ETIMEDOUT` (captured to /tmp/opencode/notes-spawnerr.json, sid ses_f66d72d3fffe7iNSsKLXTerx4v). The CLI is an HTTP client of the same background server whose request thread is blocked running that plugin code, so the request can never be served.
A blocking call back into the server from the server's own request path is a guaranteed timeout, not a slow call — and the symptom (`ETIMEDOUT` spawning your own binary) points at PATH, not at the deadlock. Prefer not needing the API at all (`ctx.location.directory`); if a call is unavoidable, make it async — `Bun.spawn` + awaited `Response`, the pattern `plugins/viz/src/index.ts` uses.

## [2026-09-13] api: /api/plugin and /api/agent report only the service's default location — location params are ignored
status: confirmed
source: live-test
evidence: against the running background service, `GET /api/plugin` returned `location.directory = /home/<user>` with 91 entries in all three forms — bare, `?location.directory=/home/<user>/Work/Notes`, and `--param location.directory=/home/<user>/Work/Notes` (identical results; 0 of 91 entries had a `source.path` under the vault). `GET /api/agent` run with cwd inside the vault omitted all four project agents (askagent, feynman, material-creator, notes-auditor, 28 entries), while `opencode2 debug agents` from that same cwd listed all 32. scripts/oc-plugins.sh consequently reported "0 failed" the whole time the vault plugin was writing notes into $HOME.
Those two endpoints are global-location-only; `?location.directory=<project>` does nothing (contrary to the advice still printed in @ocv2-pluginhealth and @ocv2-api) and an empty/clean result proves nothing about a PROJECT plugin. For project-plugin health, grep `~/.local/share/opencode/log/opencode.log`: the success line is `msg="loading plugin" id=<path>` — note the field is **`msg=`**, while every other line (incl. `message="failed to load plugin" target=<path>`) uses `message=`, so `grep 'message="loading plugin"'` returns nothing and reads like the plugin never loaded. `message="watcher subscribe" path=<…>/src/index.ts` also proves the nested files were picked up, and `message="plugin reconciliation started|completed"` brackets each reload. Or verify the side effect (the tool appearing in the session's catalog). For project agents, `opencode2 debug agents` is the cwd-scoped ground truth — it prints each agent's parsed model, system and permissions.

## [2026-09-13] agents: both `permission:` (v1 map) and `permissions:` (rule list) frontmatter are honoured
status: confirmed
source: live-test
evidence: `opencode2 debug agents` (cwd=/home/<user>/Work/Notes) — `notes-auditor` declares the plural `permissions:` rule-list form and its parsed entry carries 22 rules including its own verbatim (`edit .opencode/audit-cursor.json` allow, `shell git push*` deny, `discord-notify *` allow); `material-creator` and `askagent` declare the singular `permission:` map and it is translated (`bash {"*": ask}` → `{action:"shell",resource:"*",effect:"ask"}`, `skill {"*":allow}` → skill allow, `'*': deny` → `{action:"*",resource:"*",effect:"deny"}`). In every case the parsed list opens with the default `{action:"*",resource:"*",effect:"allow"}` and the declared rules follow.
A plural `permissions:` key is not silently inert and the singular map is not dead — do not "fix" an agent's key spelling before checking, and do not assume a deny-all is unenforced because the file looks like the other dialect. Verify what is actually applied with `opencode2 debug agents` (not `GET /api/agent`, which hides project agents).

## [2026-09-13] config: opencode.json has NO top-level `env` key — plugins and shells inherit the service's launch env
status: confirmed
source: live-test
evidence: `~/.config/opencode/opencode.json` carried `"env": {"MERMAID_DOCTOR_PLAYWRIGHT_PATH": …}`; after a clean service start (17:52, later than the edit) `tr '\0' '\n' < /proc/$(pgrep -f "opencode2 serve" | head -1)/environ` had no such variable, and neither did a shell-tool `printenv` inside a session. The plugin therefore never saw it and resolved playwright from its own hardcoded candidate (`~/.config/opencode/node_modules`, reported as `playwright@…` in the tool's output). `https://opencode.ai/config.json` agrees: `$defs.Config.properties` has 36 keys (`agent, command, formatter, lsp, mcp, permission, plugin, provider, server, shell, skills, …`) and no `env`; the only env-ish keys are `ProviderConfig.env`, `McpLocalConfig.environment`, and lsp/formatter `env`/`environment`. The loader accepts unknown top-level keys silently, so dead config still looks configured — and a `Fix: set MERMAID_DOCTOR_…` hint that assumes the config file can supply it sends the reader in circles (it did, three probe rounds).
To hand a plugin an env var, set it where `opencode2 serve` is launched (systemd user unit `Environment=`, or export before starting it). To hand a plugin a path, prefer an on-disk location it probes itself — a global plugin's `~/.config/opencode/node_modules` is already a candidate and a symlink into it works — over an env var that no one can set without restarting the service.
Related, and the reason a "forced miss" probe can silently keep passing: `os.homedir()` (bun and node) reports `$HOME` **as of process startup** — mutating `process.env.HOME` mid-process changes nothing (`before: /home/<user>` / `after mutation: /home/<user>`), while launching with `HOME=… bun script.mjs` does. So isolating a plugin's home-derived candidate dirs in a test means re-execing the probe with the env set, not assigning it at the top of the script.

## [2026-09-14] api: live "is the agent busy" needs /api/session/active + the open assistant message — /message alone lies after crashes
status: confirmed
source: live-test
evidence: `GET /api/session/active` → `{"data":{"<sid>":{"type":"running"}}}` (one call, all sessions, covers the gap before the first assistant message exists). In-progress assistant messages DO appear in `GET /api/session/:sid/message` with `time.completed` unset and streaming `reasoning`/`tool` parts — but a crashed turn leaves that state forever, so "newest assistant message incomplete" is NOT proof of activity; gate on `active` first. Phase detection must use the message's LAST part, not part presence: `reasoning` parts persist after reasoning ends (so "any reasoning → thinking" reports Thinking through the whole reply), tool parts keep `state:{status}` running/streaming/completed/error, and `{type:"tool",name,state}` plus `{type:"reasoning"|"text",text}` is the part shape. Used by the md-link thinking indicator (plugins/md-link/src/index.ts `phaseOf`).

## [2026-09-15] tui: a wrong option key in dialog.select crashes the whole TUI (options use `title`, not `label`)
status: confirmed
source: live-test + upstream-code
evidence: `ctx.ui.dialog.select({title, options:[{label, value, description}]})` took the TUI down on open with `TypeError: undefined is not an object (evaluating 'e.length')` (frame: the shared dialog-frame component, which truncates `option.title` with no guard — `titleView ?? (truncateTitle === false ? title : lI(title, titleWidth ?? 61))`). The contract is typed in `~/.config/opencode/node_modules/@opencode-ai/plugin/dist/tui.d.ts`: `TuiDialogSelectProps = {title, placeholder?, options: TuiDialogSelectOption[], flat?, current?, onMove?, onFilter?, onSelect?, skipFilter?}`, `TuiDialogSelectOption = {title, value, description?, footer?, category?, disabled?, onSelect?}`; the promise wrapper resolves with the selected option's `value` and `undefined` on esc. This is the same `label`→`title` rename that broke session forms on 2026-08-27 (forms entry, GOTCHA 5) — the TUI convention is `title` everywhere, `label` is the old/HTTP-forms-wording. Plugin code passes `ctx: any`, so nothing typechecks it: a mistyped dialog prop is a runtime crash of the user's whole terminal, not a caught error. Always read `tui.d.ts` before guessing a dialog prop, and prefer a dialog call whose options came from a typed local.

## [2026-09-19] mcp: hot reload fires on opencode.json content change only, wipes runtime /mcps enables, and local servers cache config at spawn
status: confirmed
source: live-test
evidence: editing `~/.config/opencode/opencode.json` re-logged `mcp connected` within ~5s for every project dir (2.0.9); a bare `touch` (mtime-only) did nothing. A runtime enable done via the TUI was LOST on that reload — the server dropped from Code Mode's catalog until `enabled: true` was written into the config. A local stdio MCP (`browser-use --mcp`) reads its own config file (`load_browser_use_config()` in `__init__`) once at spawn, so fixing the server's config on disk does nothing until the MCP process respawns — force it via any opencode.json content edit. Also: `opencode mcp list` NotFound on a local server can mean a dead venv interpreter symlink (uv pruned cpython-3.12.10 out from under `.venv/bin/*` shebangs) — the named script exists; check `head -1 <script>` + `readlink -f`.
