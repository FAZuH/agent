#!/usr/bin/env bash
# agent — installer. Symlinks repo items into an OpenCode config dir.
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
GLOBAL_CFG="$HOME/.config/opencode"
AGENTS_SKILLS="$HOME/.agents/skills"
MANIFEST="$REPO/.agent-links.json"
DRY=0
FORCE=0
...
MODE=""       # "global" | "project"
TOPS=(agents skills plugins commands)
...
usage() {
  cat <<EOF
${B}agent — installer${R}
Symlinks this repo's items (skills, agents, plugins, commands) into an OpenCode
...
${B}Notes:${R}
  ...
  - Created links are tracked in .agent-links.json (gitignored);
    uninstall.sh removes exactly those.
  - This is the ONLY supported way to install these items on this machine.
    Do NOT also npx-install fazuh/agent: the copies in ~/.agents/skills would
    be silently shadowed by the config root and flagged as collisions.
...
if [[ -f "$MANIFEST" ]]; then
  ...
  AGENT_DRY=$DRY python3 - "$MANIFEST" "$TARGET_KEY" "$REPO/" <<'EOF' | while IFS= read -r link; do
import json, os, sys
manifest_path, key, repo_prefix = sys.argv[1:4]
dry = os.environ.get("AGENT_DRY") == "1"
try:
    m = json.load(open(manifest_path))
except Exception:
    raise SystemExit
...
# The config/project skill roots take precedence over ~/.agents/skills, so any
# same-named copy there is dead weight that drifts and trips collision checks.
# Advisory only — this script never deletes anything it did not create.
shadowed=()
for rel in "${items[@]}"; do
  [[ "$rel" == skills/* ]] || continue
  name="${rel#skills/}"
...
if [[ ${#shadowed[@]} -gt 0 ]]; then
  ...
  printf '  %s\n' "${shadowed[@]}"
  warn "the installed symlink wins; delete the copies or skill-doctor will keep
        flagging collisions/drift:"
...
if [[ $DRY -eq 0 ]]; then
  python3 - "$MANIFEST" "$TARGET_KEY" "$TARGET" "$REPO" "${items[@]}" <<'EOF'
import json, os, sys
